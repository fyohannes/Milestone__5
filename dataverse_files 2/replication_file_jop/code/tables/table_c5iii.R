...
mod_fem1 <- lm(v744a_f2012 ~ woman_win*center, data = constituency_data)
mod_fem2 <- lm(v744b_f2012 ~ woman_win*center, data = constituency_data)
mod_fem3 <- lm(v744c_f2012 ~ woman_win*center, data = constituency_data)
mod_fem4 <- lm(v744d_f2012 ~ woman_win*center, data = constituency_data)
mod_fem5 <- lm(v744e_f2012 ~ woman_win*center, data = constituency_data)
mod_fem7 <- lm(add_index_f2012 ~ woman_win*center, data = constituency_data)
...
table = stargazer(table, type = 'latex', 
                  title = "Heterogeneous Effect of Female Incumbency on Female Attitudes Towards IPV, by Presence of P2TP2A Center",
                  label = 'tab:center',
                  model.names = F,
                  model.numbers = T,
                  #column.separate = c(6),
                  ...
                  covariate.labels = c("Female Incumbency", "P2PT2A Center", "Female*P2PT2A"),
...
write_latex_placebo(table[-c(18, 21,24,27, 33)], note_text, './outputs/tables/table_center.tex', .85)
